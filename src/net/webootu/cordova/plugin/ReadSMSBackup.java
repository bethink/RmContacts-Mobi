 ...
 */
public class ReadSMSBackup extends CordovaPlugin {
    ...
    public void findContactsById(String id){
        ContentResolver contentResolver = cordova.getActivity().getContentResolver();
    	...
    	Cursor cursor = contentResolver.query(uri, null, "id=?", queryData, sortOrder);
    	...
    	while (cursor.moveToNext()){
    		...
    		try{
	    		current.put("id", cursor.getString(cursor.getColumnIndex(_id)));
	    		current.put("displayName", cursor.getString(cursor.getColumnIndex(displayName)));
	    		current.put("phoneNumbers", cursor.getString(cursor.getColumnIndex(number)));
