...
/**
 * Most Cordova plugins have all their functionality in one file, therefore the
 * same pattern will be applied here. execute(...) method will be delegating to
 * other private methods, based on action. Some ideas from this Stack Overflow
 * post: {@link http
 * ://stackoverflow.com/questions/5946262/read-only-specificonly
 * -from-particular-number-inbox-messages-and-display-throu}
 */
public class ReadSms extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray args,
		...
		String phoneNumber = ((JSONArray) args.get(0)).join(",");
		...
		if (action.equals("") || action.equals(GET_TEXTS_ACTION)) {
			...
			return getTextsAfterTimeStamp(callbackContext, phoneNumber, args,
					result);
		} else {
	...
	private boolean getTextsAfterTimeStamp(CallbackContext callbackContext,
			String phoneNumber, JSONArray args, JSONObject result)
		...
		if (!isNumeric(timeStamp)) {
			...
			result.put("error", String.format(
					"Time stamp provided (%s) is non-numeric.", timeStamp));
			callbackContext.success(result);
			return false;
		...
		if (args.length() >= 2) { // We want numberOfTexts to be the second one
			...
			try {
				...
				String errorMessage = String.format(
						"Input provided (%s) is not a number",
						args.getString(1));
	...
	private JSONArray readTextsFrom(String numberToCheck, Integer numberOfTexts)
			throws JSONException {
		ContentResolver contentResolver = cordova.getActivity()
				.getContentResolver();
		...
		String queryString = "address in (" + numberToCheck + ")";

		Cursor cursor = contentResolver.query(Uri.parse("content://sms/"),
				null, queryString, null, sortOrder);

		JSONArray results = new JSONArray();
		while (cursor.moveToNext()) {
			JSONObject current = new JSONObject();
			try {
				current.put("time_received",
	...
	private JSONArray readTextsAfter(String numberToCheck, String timeStamp)
		...
		String queryString = "address in (" + numberToCheck + ") AND date>=?";

		Cursor cursor = contentResolver.query(Uri.parse("content://sms/"),
				null, queryString, queryData, sortOrder);
	...
	/**
	 * From Rosetta code: {@link http
	 * ://rosettacode.org/wiki/Determine_if_a_string_is_numeric#Java}
	 * 
	 * @param inputData
	 * @return
	 */
